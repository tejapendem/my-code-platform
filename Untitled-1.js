...
const RoadmapPage = () => {
  ...
  try {
    ...
    const newRoadmap = res.data.roadmap; // this is a single object

    // prepend new roadmap object to existing array
    setRoadmap([newRoadmap, ...roadmap]); 
  } catch (err) {
    console.error(err);
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-white/70 dark:bg-slate-900/70 backdrop-blur shadow-md">
        <nav className="max-w-6xl mx-auto px-6 py-4 flex justify-between items-center">
          <ul className="flex gap-6 text-gray-700 dark:text-gray-200 font-medium">
            <li className="text-blue-600 font-semibold">Roadmaps</li>
      ...
      <div className="pt-24 max-w-6xl mx-auto px-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl p-6 shadow-lg">
            ...
            <select
              className="border p-2 rounded w-full mb-3 bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
              value={technologies}
          ...
          {/* Right: My Roadmaps Card */}
          <div className="bg-gray-100 dark:bg-slate-800 rounded-xl p-6 shadow-lg">
            <h2 className="text-xl font-bold mb-4 text-gray-700 dark:text-gray-200">
              My Roadmaps
            </h2>

            {roadmap.length === 0 && (
              <p className="text-gray-500">No roadmaps yet. Generate one!</p>
            )}
            ...
            <div className="space-y-4 max-h-[500px] overflow-y-auto">
              {roadmap.map((rm, idx) => (
                <div
                  key={rm._id || idx}
                  className="p-3 border rounded bg-white dark:bg-slate-700"
                >
                  <h4 className="font-semibold">{rm.technology}</h4>
                  <p className="text-sm text-gray-500">
                    {rm.duration} • {rm.difficulty}
                  </p>
                  <ul className="list-disc ml-5 mt-2">
                    {rm.modules?.map((mod, i) => (
                      <li key={i}>
                        {mod.name || mod.step || JSON.stringify(mod)}
                      </li>
                    ))}
                  </ul>
                </div>
